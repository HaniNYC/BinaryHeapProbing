...
//     }
    ...
    void increaseKey(int delta, Comparable key)
    {
    ...
    void decreaseKey(int delta, Comparable key)
    {
	Node newNode(key,0,0);
	int loc = hashTable_.findLocation(newNode);
	array_[loc].delta -= delta;
	...
	{
	int loc = hashTable_.findLocation(newNode);
	array_[loc].delta = std::numeric_limits<int>::min(); // minimum value of int
	newNode = percolateUp(loc);
	deleteMin();
	}
	// else do nothing
	...
	int delta;
	int location;
	Node(Comparable newkey, int newdelta, int newlocation) : key(newkey), delta(newdelta), location(newlocation) {}
    };
   private:
